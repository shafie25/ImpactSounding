...
#
# Loads Labeled_Features.xlsx, trains an XGBoost classifier,
# evaluates performance, and saves results + plots to
# Classifier Results/XGBoost/
# ============================================================
...
n_intact   = int((y == 1).sum())
n_cracked  = int((y == 0).sum())
scale_pos  = n_intact / n_cracked   # ~4.6
...
clf = XGBClassifier(
    ...
    learning_rate=0.1,
    max_depth=6,
    scale_pos_weight=scale_pos,   # compensates for class imbalance
...
ax.barh(top_names[::-1], top_vals[::-1], color="darkorange")
...
ax.set_ylim(0, 1.1)
ax.set_ylabel("Score")
ax.set_title("XGBoost — 5-Fold CV Scores (mean +/- std)")
